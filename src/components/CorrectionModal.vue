<template>
  <div class="correction-info-panel">
    ...
    <div class="correction-inputs">
      <div class="point-inputs">
        <div class="point-with-prefix">
          ...
          <input class="graph-input" id="x1" :value="point1x" @change="updateX1" :disabled="previewEnabled" />
        </div>
        <div class="point-with-prefix">
    ...
    <div class="correction-actions">
      <button class="correction-button" @click="applyCorrection" :disabled="imgCorrected">
        Применить
      </button>
      <button class="correction-button" @click="resetValues">Сбросить</button>
    </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    //Анализирует пиксели изображения, создает гистограммы
    buildGraph() {
      const img = new Image();
      img.onload = () => {
    ...
    // Создает таблицу LUT (Look-Up Table)
    calculateCorrection() {
      try {
        const lut = [];
